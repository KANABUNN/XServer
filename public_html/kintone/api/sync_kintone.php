...
try {
    $action = (string)($_POST['action'] ?? 'sync_all');
    if ($action === 'retry_failed') {
        $sourceJobId = (int)($_POST['source_job_id'] ?? 0);
        $result = kintone_sync_retry_failed_job($sourceJobId, $user);
        kintone_set_flash('success', '失敗分の再同期ジョブを作成しました。ジョブID: ' . $result['job_id'] . ' / 成功: ' . $result['success'] . ' / 失敗: ' . $result['failed']);
    } elseif ($action === 'sync_selected') {
        $ids = $_POST['organization_ids'] ?? [];
        if (!is_array($ids)) {
            $ids = [];
        }
        $result = kintone_sync_organizations($ids, $user, 'manual');
        kintone_set_flash('success', '選択団体のkintone同期が完了しました。ジョブID: ' . $result['job_id'] . ' / 成功: ' . $result['success'] . ' / 失敗: ' . $result['failed']);
    } else {
        $result = kintone_sync_organizations([], $user, 'manual');
        kintone_set_flash('success', '活動中団体のkintone同期が完了しました。ジョブID: ' . $result['job_id'] . ' / 成功: ' . $result['success'] . ' / 失敗: ' . $result['failed']);
    }
...
header('Location: ../sync.php', true, 302);
exit;
